...
export function generateStaticParams() {
  return PROBLEM_INTENT_IDS.map((id) => ({ intent: id }))
}
...
export async function generateMetadata({
  ...
  const items = (dict.problemIntents?.items ?? {}) as Record<
    string,
    { title: string; desc: string }
  >
  const copy = items[intent]
  const { buildPageMetadata } = await import("@/lib/seo/metadata")
  return buildPageMetadata({
    locale,
    path: `/zrob/${intent}`,
    title: `${copy?.title ?? intent} — Toolando.tech`,
    description: copy?.desc ?? "",
  })
}

export default async function ProblemIntentPage({
  params,
}: {
  params: Promise<{ locale: string; intent: string }>
}) {
  const { locale, intent } = await params
  const toolkit = getProblemIntent(intent)
  if (!toolkit) notFound()
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-4xl px-4 pb-24 pt-32">
        ...
        <p className="mt-6 text-sm font-medium text-primary">
          {dict.problemIntents?.eyebrow ?? "Solve a file problem"}
        </p>
