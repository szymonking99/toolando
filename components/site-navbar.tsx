...
export function SiteNavbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-4 sm:px-4">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-2xl border border-white/10 bg-background/60 px-4 py-3 backdrop-blur-xl">
        ...
        {/* Desktop links */}
        <div className="hidden items-center gap-3 whitespace-nowrap min-[900px]:flex lg:gap-6 xl:gap-8">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
        ...
        {/* Desktop right side */}
        <div className="hidden items-center gap-2 min-[900px]:flex lg:gap-3">
          <LanguageSwitcher />
          <AccountNavButton />
          <SupportButton />
        </div>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="mx-auto mt-2 max-w-7xl rounded-2xl border border-white/10 bg-background/80 p-4 backdrop-blur-xl min-[900px]:hidden">
